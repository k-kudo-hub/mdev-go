...
以下は実施の過程で確定したもので、Decision の一部として扱う。

### D8-1. Shell 版最終リリースのタグ

**ロールバック先は claude-conductor の `v0.9.1`** とする。
...
これが Shell 実装として完結している最後のタグである。6-4 で作る最終リリースは
「後継の告知」だけを載せた patch であり、実装は含まない。**戻り先としては
v0.9.1 を指す**のが正しい(告知だけのタグへ戻しても Shell 版は動くが、
最終リリースの番号を指す意味が無い)。

告知リリースの番号は 6-4 の完了時にここへ追記する。

### D8-4. ADR-0001 のデータ形式凍結の解除

**解除する。** ADR-0001 は「Shell 版と Go 版が同じデータを読み書きする」ことを
前提に、pending / registry / daily の JSON 形式を凍結していた。両方が同時に
動く状況が無くなった以上、この制約を維持する理由が消える。

ただし **凍結の解除は「自由に変えてよい」ではない**。

- 既存のデータは読めなければならない。daily は作業ログそのもので、失うと
  取り返しがつかない
- 形式を変える場合は、古い形を読める経路を残すか、移行を `mdev install` に
  含める(利用者が手で直す作りにはしない)
- golden fixture は **Shell 版との一致の記録として残す**。conductor を
  アーカイブすると再生成できないため、期待値を変えるときは何を根拠に変えたかを
  コミットに残す(各 gen-golden-*.sh の冒頭に同じ注記がある)
...
- ✅ ADR-0001 のデータ形式凍結が解除され、内部形式を必要に応じて進められる(移行は install が担う)
