...
| daily ログの `restored: true` の付け方 | `jq -c` でファイル全体を出し直す | 対象行にキーを差し込むだけ | 触っていない行の表記(数値の桁など)を変えない |
...
- [ ] codex の diff ビューアなどを開いても、Dashboard の表示が変わらない
...
引き継がれていない場合は transcript が消えている可能性がある。
...
worktree を閉じたなど、記録された `dir` が無いタスクがある状態で作り直す。
