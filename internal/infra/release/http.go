...
// downloadTimeout は取得を諦めるまでの時間である。
//
// 相手はリリースのアセット(十数 MB)で、まともな回線なら数秒で終わる。
// 60 秒かかる時点で異常だが、遅い回線でも一度は成功してほしいので短くは
// しない。
const downloadTimeout = 60 * time.Second

// maxDownloadBytes は受け取るファイルの上限である。
//
// 応答が壊れて延々と流れてきたときにディスクを埋めないための歯止めである。
// 上限に達したら **必ず error にする**。黙って切り詰めると、途中までの
// バイナリを置き換えてしまい、更新に失敗したことに気づけない。
