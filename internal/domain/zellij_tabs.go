...
// タブは含まない。スペースが 2 回見つからない行と、落とした後に何も残らない
// 行はそのまま返す。
//
// awk と結果が違うのは**行頭がスペースで始まる行**だけである。awk は `[^ ]+`
// が先頭で一致せず行をそのまま返すのに対し、この実装は行頭の空白列を 1 つ目の
// 区切りとして数えて先へ進む。`zellij action list-tabs` は行頭にスペースを
// 置かないため実際には到達せず、到達しても ResolveTabID はどちらの結果でも
// 一致せず空文字を返すため、誤ったタブを閉じることはない(検証済み)。
