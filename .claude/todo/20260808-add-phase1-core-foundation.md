...
- [x] `HandleNotify`(pending-notify.sh 相当)のテストを作成(session_id 空で no-op / registry の AND ガード / 上書き規則の適用)→ 実装
- [x] `HandlePostTool` のテストを作成(Notification のみ削除 / Main フォーカス / registry 不触)→ 実装
- [x] `HandleResolve` のテストを作成(無条件削除 / upsert / pending 無しでもフォーカス)→ 実装
...
- [x] mkdir ロックのテストを作成(獲得・解放 / stale 検出 / タイムアウト 2 秒 / fail-open)→ 実装
- [x] Zellij Focuser adapter を実装(`zellij action go-to-tab-name`、エラーは無視 = 現行挙動)
