...
- tui 側もユースケースを `DashboardService` / `WaitingService` / `DoneService` /
  `NewsService` の interface として定義した。
...
## 10. /code-review 指摘への対応

PR #5 に対する `/code-review` の指摘を検証した結果と対処。指摘は重大度順。

### 10-1. 2 打鍵プロンプト中の TOCTOU(最重大)

**指摘**: `d` → 数字の待ち受け(3 秒)の間もポーリングが一覧を差し替えるため、
押した番号が別のタブを指し、消すつもりのないタブが消える。

**検証**: 再現した。テストで「待ち受け中に別の一覧を運ぶ読み直しを着弾させ、
その後 `1` を押す」と、`prepare gamma`(入れ替わった側の 1 番)が呼ばれた。
現行 Shell 版は `read -t 3` がループを止めるため、この隙間が構造的に無い。

**対処**: 待ち受け中はポーリングを凍結する(Shell 版と同じ「表示も番号対応も
止まる」意味にする)。

| 経路 | 待ち受け中の扱い |
|------|------------------|
| tickMsg | 読み直さず、次の合図だけを予約する |
| refreshedMsg(発行済みの在庫) | 適用しない(捨てる) |
| 待ち受けの解決(時間切れ / 無効キー / 範囲外) | 凍結を解いて読み直す |

Dashboard と Done の両方に同じ構造を入れた。テストは
`TestDashboardModelFreezesTabsWhileAwaiting` /
`TestDoneModelFreezesRowsWhileAwaiting`(外部)と、`tickMsg` が非公開型のため
`tick_internal_test.go`(内部)で固定している。

### 10-2. ScreenDetectTick の %q 埋め込み

**指摘**: `fmt.Sprintf(". %q; ...", path)` は shell のエスケープではないため、
`$` やバッククォートを含む CONDUCTOR_HOME でコマンド実行が起きる。

**検証**: 実際の bash で再現した。`h $(touch pwned)` を含むディレクトリを
CONDUCTOR_HOME にすると `pwned` が作られ、source 先のパスも別物に化けた。

**対処**: `bash -c '. "$1"; screen_detect_tick "$2"' _ <path> <session>` の
位置パラメータ方式に統一した(Runner の他のメソッドと同じ argv 渡し)。
テストは実 bash を起動して、コマンドが実行されないこととライブラリが正しく
source されることの両方を確かめる。

### 10-3. エラーの未描画 + ゼロスナップショット上書き

**指摘**: `m.err` は書き込むだけで描画されず、読み直しに失敗すると一覧が
ゼロ値で上書きされて無言の白画面になる。削除の前半の失敗も何も出ない。

**検証**: 再現した(失敗後の `View()` が空文字)。

**対処**:

- 読み直しの失敗は直前の内容を保ち、本文の下へ赤字のエラー行を 1 行足す。
  復帰したらエラー行は消える(Dashboard / Waiting)
- 削除の前半・後半の失敗は、中止(`Upload failed.`)と同じ通知行として
  数秒出す。これで削除フローの 3 つの失敗経路が同じ形になった
- Done / News は失敗しうる呼び出しを持たない(集計は読めたファイルだけで作り、
  restore-task.sh / fetch-news.sh / ブラウザ起動の終了コードは現行版と同じく
  見ない)ため、エラー行を持たない。その旨をコメントに残した
- 単発描画(`--once`)はエラーを戻り値で返す経路のままなので、ゴールデンの
  出力は変わらない

現行 Shell 版にはこの表示に対応するものが無い(そもそも失敗を検出しない)。
挙動差として 6 章の一覧と同じ扱いで記録する。

### 10-4. ポーリングチェーンの増殖

**指摘**: refreshedMsg が無条件に次の合図を予約する一方、tick 以外の生成元が
5 つあるため、キー操作のたびにチェーンが 1 本増える。

**検証**: 再現した。ジャンプを 10 回押すと 2 秒ごとの Refresh が 11 本になり、
その本数ぶん bash と zellij のプロセス生成が多重に走る。

**対処**: 予約するのは Init(開始)と tickMsg(張り直し)だけにした。
tickMsg は `tea.Batch(refreshCmd, tickCmd)` を返す。4 ペインすべて。
テストは「Init が返すコマンドは 2 つ」「読み直しの完了はコマンドを返さない」を
4 ペインぶん固定する(`TestPaneInitStartsPollingOnce` /
`TestPaneRefreshDoesNotReschedulePolling`)。10-1 の凍結もこの構造の上に乗る。

### 10-5. ResolveTabID の複数一致

同じ名前のタブが複数あると `"1\n3"` を返し、`close-tab` がそれを id として
解釈できずタブが閉じ残る。pending とレジストリは先に消えているので、一覧から
消えたのにタブだけが残る。現行 Shell 版も同じ経路で壊れるが、Go 版は
**先頭の一致だけを返す**改善に倒した。少なくとも 1 つは閉じられ、残りは次の
削除操作で閉じられる。6 章の「意図的な差異」に相当する。

### 10-6. jqToString の重複

`pane_pending.go` の `jqRawString` とほぼ逐語のコピーだった。挙動が違うのは
「JSON として解釈できない入力」と「Compact に失敗する入力」のときだけで、
どちらの呼び出し元も `jqIndex` が解釈済みの値を渡すため到達しない。
`jqToString` を削除して `jqRawString` に寄せ、ゴールデン 23 ケースが引き続き
全通過することを確認した。

### 10-7. ドキュメントの曖昧語

`docs/user-test-02-panes.md` の 3 箇所と本ファイルの 1 箇所の「など」を、
具体的な列挙または言い切りへ書き換えた。

### 10-8. REFUTED(検証の結果、問題ではなかったもの)

3 件は検証の結果、指摘が成り立たないと判断して除外した。

**設定の読み込みエラーを握り潰している**
`store/pane.go` の `Load` が壊れた設定でゼロ値へ落ちる点。現行 task-lib.sh の
`agent_detection` が `jq ... 2>/dev/null` で失敗を握り潰して検出方式を既定の
`hooks` に落とすのと同じ挙動で、コメントと
`TestPaneStoreLoadBrokenConfigFallsBackToHooks` で意図として固定済みである。
検出方式を取り違えても次のポーリングで pending が作り直されるため状態は
失われない。残る改善余地はフォールバック時にログを 1 行出す程度。

**jq の数値フォーマットが科学記法で食い違う**
`formatJQNumber` の `'g'` と jq の表記差は実在するが、整数の fast-path があり
実データはすべてそこを通る(金額は `'f', 2` の別処理、pending の値は書き手側で
すべて文字列)。float64 の分岐は防御的なコードで到達しない。

**stripTwoColumns が awk と食い違う**
行頭がスペースで始まる行での機械的な差異は実在する(awk は行をそのまま返し、
Go 実装は空白列を 1 つ目の区切りとして数える)。ただしどちらの結果でもタブ名と
一致せず `ResolveTabID` は空文字を返すため、誤ったタブを閉じることはない。
指摘のうち妥当だったのはコメントの不正確さ(「パターンに一致しない行はそのまま
返す」が空白始まりの行で成り立たない)だけで、コメントを実際の挙動に直し、
空白始まりの行が一致しないことをテストで固定した。
