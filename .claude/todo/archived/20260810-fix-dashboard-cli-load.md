...
- [x] 指摘 3・8: ポーリングの回し方を `poller` 型へ集約し、実行中を真偽値ではなく本数で数える(キー操作とポーリングの交錯で desync しない)
- [x] 指摘 4: `return m, m.forceRefreshCmd()` の評価順ハザードを別文へ分離
- [x] 指摘 1・2: ポーリングと起動の外部コマンドに実行時間の上限(スクリーン検出 15 秒 / zellij CLI 10 秒 / セッション復元 60 秒)。返らない呼び出しでチェーンが死なないようにする
- [x] 指摘 5: 設定を読めなかったときはスクリーン検出を止めない(fail-open)。`ConfigLoader.Load` が ok を返す
- [x] 指摘 6: 2 打鍵目を受け取ったら打ち切りタイマーの世代を進める(削除処理中の古い発火を無視)
- [x] 指摘 7: 取得中のニュース画面をポーリングの着弾で差し替えない(二重取得の防止)
- [x] evidence に指摘対応の章(否定した懸念 2 件を含む)を追記

## 完了条件

- in-flight 中の tick が refresh を発行しないことがテストで固定されている
- ポーリングのチェーンが常にちょうど 1 本であることがテストで固定されている
- キー操作とポーリングの読み直しが交錯しても重ならないことがテストで固定されている
