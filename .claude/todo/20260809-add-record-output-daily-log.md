...
- [ ] claude transcript 集計のテストを作成(turns / tool_calls / tools_used / model / speed / トークン 5 種。test.sh の期待値を移植)→ 実装
- [ ] pricing 解決と cost 計算のテストを作成(モデル別 → sonnet フォールバック → ハードコード既定、fast_multiplier、丸め)→ 実装
