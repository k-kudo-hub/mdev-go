...
- [ ] domain: ps 出力からのゾンビ検出のテスト+実装(zellij --server プロセスと list-sessions の突き合わせ、PPID=1 の zellij action クライアント検出)
...
- [ ] domain: 減速判定の純粋関数のテスト+実装(attach 確認は 30 秒間隔、未アタッチが確認できたら poll 間隔を通常 → 60 秒へ、attach 復帰で即通常へ)
